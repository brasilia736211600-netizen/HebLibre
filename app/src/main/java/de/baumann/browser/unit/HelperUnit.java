...
public class HelperUnit {
    ...
    public static void makeBackupDir (final Activity activity) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= 23 && android.os.Build.VERSION.SDK_INT < 29) {
            ...
            if (hasWRITE_EXTERNAL_STORAGE != PackageManager.PERMISSION_GRANTED) {
                ...
                try { backupDir.mkdirs(); } catch (Exception e) { e.printStackTrace(); }
            }
        } else if(!backupDir.exists()) {
            try { backupDir.mkdirs(); } catch (Exception e) { e.printStackTrace(); }
        }
    ...
    public static void applyTheme(Context context) {
        ...
        switch (showNavButton) {
            case "0": context.setTheme(R.style.AppTheme_system); break;
            case "2": context.setTheme(R.style.AppTheme_dark); break;
            case "3": context.setTheme(R.style.AppTheme_amoled); break;
    ...
    public static void save_as (final Activity activity, final String url) {
        try {
            ...
            if(extension.length() <= 8) editExtension.setText(extension);
                    ...
                    if (title.isEmpty() || extension.isEmpty() || !extension.startsWith(".")) {
                        ...
                        if (android.os.Build.VERSION.SDK_INT >= 23 && android.os.Build.VERSION.SDK_INT < 29) {
                            ...
                            if (hasWRITE_EXTERNAL_STORAGE != PackageManager.PERMISSION_GRANTED) {
                                ...
                                if (DownloadCookiePolicy.shouldSendCookies(PreferenceManager.getDefaultSharedPreferences(activity)
    ...
    public static void setBottomSheetBehavior (final BottomSheetDialog dialog, final View view, int beh) {
        ...
        mBehavior.addBottomSheetCallback(new BottomSheetBehavior.BottomSheetCallback() {
            @Override public void onStateChanged(@NonNull View bottomSheet, int newState) { if (newState == BottomSheetBehavior.STATE_HIDDEN) dialog.cancel(); }
        ...
        try {
            ...
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
                ...
                if (shortcutManager.isRequestPinShortcutSupported()) {
                    ShortcutInfo pinShortcutInfo = new ShortcutInfo.Builder(context, url)
                            .setShortLabel(title).setLongLabel(title)
                            .setIcon(Icon.createWithResource(context, R.mipmap.qc_bookmarks))
                            .setIntent(new Intent(Intent.ACTION_VIEW, Uri.parse(url))).build();
                    shortcutManager.requestPinShortcut(pinShortcutInfo, null);
                } else { System.out.println("failed_to_add"); }
            }
        } catch (Exception e) { System.out.println("failed_to_add"); }
    }
    ...
    public static String domain (String url) {
        if(url == null) return "";
        try { return Objects.requireNonNull(Uri.parse(url).getHost()).replace("www.", "").trim(); }
        catch (Exception e) { return ""; }
    }

    public static SpannableString textSpannable (String text) {
        SpannableString s;
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.N) s = new SpannableString(Html.fromHtml(text,Html.FROM_HTML_MODE_LEGACY));
        else s = new SpannableString(Html.fromHtml(text));
    ...
    public static void initRendering(WebView webView) {
        if (sp.getBoolean("sp_invert", false)) {
            ...
            if(WebViewFeature.isFeatureSupported(WebViewFeature.FORCE_DARK)) WebSettingsCompat.setForceDark(webView.getSettings(), WebSettingsCompat.FORCE_DARK_OFF);
        }
    }
    ...
    public static void setFilterIcons (ImageView ib_icon, long newIcon) {
        if (newIcon == 11) ib_icon.setImageResource(R.drawable.circle_red_big);
        else if (newIcon == 10) ib_icon.setImageResource(R.drawable.circle_pink_big);
        else if (newIcon == 9) ib_icon.setImageResource(R.drawable.circle_purple_big);
        else if (newIcon == 8) ib_icon.setImageResource(R.drawable.circle_blue_big);
        else if (newIcon == 7) ib_icon.setImageResource(R.drawable.circle_teal_big);
        else if (newIcon == 6) ib_icon.setImageResource(R.drawable.circle_green_big);
        else if (newIcon == 5) ib_icon.setImageResource(R.drawable.circle_lime_big);
        else if (newIcon == 4) ib_icon.setImageResource(R.drawable.circle_yellow_big);
        else if (newIcon == 3) ib_icon.setImageResource(R.drawable.circle_orange_big);
        else if (newIcon == 2) ib_icon.setImageResource(R.drawable.circle_brown_big);
        else if (newIcon == 1) ib_icon.setImageResource(R.drawable.circle_grey_big);
    }
}
