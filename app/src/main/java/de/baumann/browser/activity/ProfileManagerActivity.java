...
/**
 * Lightweight profile catalog UI. Browser/WebView replacement remains a
 * separate lifecycle step because a WebView profile cannot be rebound after
...
public class ProfileManagerActivity extends AppCompatActivity {
    ...
    private void renderProfiles() {
        ...
        for (final ProfileMetadata profile : profiles) {
            ...
            String marker = profile.getId().equals(activeId)
    ...
    private void selectProfile(String profileId) {
        ...
        if (current.equals(ProfileIdentity.normalize(profileId))) {
            return;
        }
    ...
    private void showEditor(final ProfileMetadata existing) {
        ...
        final EditText name = editText(R.string.profile_name, existing == null ? "" : existing.getName());
        fields.addView(name);
        final EditText color = editText(R.string.profile_color, existing == null ? "" : existing.getColor());
        fields.addView(color);
        final EditText icon = editText(R.string.profile_icon, existing == null ? "" : existing.getIcon());
        fields.addView(icon);
        final EditText notes = editText(R.string.profile_notes, existing == null ? "" : existing.getNotes());
        fields.addView(notes);
        final EditText tags = editText(R.string.profile_tags,
                existing == null ? "" : join(existing.getTags()));
        fields.addView(tags);
        final EditText group = editText(R.string.profile_group, existing == null ? "" : existing.getGroup());
                    ...
                    public void onClick(View v) {
                        ...
                        if (profileId.isEmpty() || profileName.isEmpty()) {
                            Toast.makeText(ProfileManagerActivity.this,
                                    R.string.profile_required_fields, Toast.LENGTH_SHORT).show();
                            return;
                        }
                        ...
                        if (!rawTags.trim().isEmpty()) {
                            tagValues.addAll(Arrays.asList(rawTags.split(",")));
                        }
