...
## Privacy controls
### Media permissions
`WebRtcPermissionPolicy` blocks WebView camera/microphone capture resources when `block_media_permissions` is enabled. This is intentionally narrower than full WebRTC engine privacy control. Default is enabled.
...
### Geolocation
`GeolocationPermissionPolicy` now gates `NinjaWebChromeClient.onGeolocationPermissionsShowPrompt()` on the canonical `SP_LOCATION_9` preference. Disabled means the WebView callback is denied; enabled preserves the existing runtime permission request flow. This closes a previously source-visible mismatch where geolocation could be granted despite the setting being disabled.
...
## Current checkpoint
P2.1–P2.9 are CI-VERIFIED. P2.7/P2.8 feature work is proven by Unit Tests run `33679583870`; P2.9 geolocation privacy guard is proven by run `33684710168`. Android runtime remains reserved for final validation.

## Current phase
`P2 — WebLibre Feature Gap Implementation`

## Last synchronized
2026-09-02 — P2.9 geolocation privacy guard reconciled through successful CI; autonomous execution, final-device validation, and concise checkpoint-reporting rules remain mandatory.
