...
`ee7f97c4a76dcf8bc19e897b0129ccc52938d9ba` — latest documentation-synchronized branch HEAD.

## Current source checkpoint
`b3086e0b7c939a4c43936acba5b72e9bec132c7b` — profile-aware records/session restore, profile transfer with optional AES-GCM encryption, hardened transfer parsing, transactional profile-record import, atomic profile/privacy-rule deletion, and profile-local browser/privacy preference switching + transfer coverage.
...
- `ProfileCatalogStore`: local SharedPreferences catalog persistence, metadata persistence, active-profile validation, and safe fallback to the default profile. Profile switching saves outgoing curated browser/privacy settings and loads the incoming profile's settings through the legacy global preference contract.
...
- `ProfileTransferActivity`: Android document picker, encrypted export password, password not persisted, WebView internal cookies/storage/login secrets excluded, conflicting/reserved imported IDs require a new ID. Curated profile-owned browser/privacy settings are now included in plain and encrypted transfer.
- `ProfilePreferencesStore`: curated per-profile browser/privacy settings bridge with deterministic typed snapshots/restoration. Unknown or malformed imported entries are ignored. The controlled set currently covers browser/privacy settings explicitly inspected in the settings resources, not UI-only theme/gesture/filter options.
...
- `RecordAction.deleteProfileRecords(...)` plus Profile Manager integration: deleting a user profile now purges app-owned HISTORY/BOOKMARK/TAB and profile-scoped WHITELIST/JAVASCRIPT/COOKIE/REMOTE rows in one SQLite transaction before removing the catalog entry.
...
- Runtime smoke coverage: `ProfileManagerSmokeActivity` verifies rollback behavior after a deliberately invalid imported record, profile-record isolation, whitelist isolation, deletion purge across all profile-scoped app-owned tables, profile-preference isolation, and profile-preference transfer snapshot/restore.

## Verification evidence
- SOURCE-VERIFIED: current branch source is synchronized at `ee7f97c4a76dcf8bc19e897b0129ccc52938d9ba`; implementation changes listed above are present in the branch.
- TEST-VERIFIED: prior Unit Tests runs `33994575842` and `33990897505` passed on earlier checkpoints. Fresh deterministic test execution for the current checkpoint remains unverified because hosted jobs terminate before executing a step.
- CI-VERIFIED: latest current Runtime Smoke run `33999892923` on checkpoint `0ef058381decd4bb2c8698d443da0ca1c7ab54e2` failed before any step; the dedicated Runner Probe also failed before any step. Current CI failure therefore remains runner allocation/startup, not an application test result.
- ANDROID-RUNTIME-VERIFIED: earlier Android Runtime Smoke run `33994758706` on checkpoint `5aa5a6e87e45b0fa3cc7aca820c1de6b8bfbdb8e` completed successfully for the baseline browser/emulator flow. New profile-preference smoke coverage has not yet executed on a fresh runner.
- ARTIFACT-VERIFIED: an earlier GitHub Actions x86_64 smoke APK matched its published SHA-256 checksum. No current artifact exists while the runner cannot start.
- DOCUMENTED: yes. `01_STATE.md` and `02_CONTINUATION_2026-09-06.md` are synchronized to the latest state.
...
- User-profile deletion purges app-owned HISTORY/BOOKMARK/TAB and profile-scoped privacy-rule tables before catalog removal in one SQLite transaction; it does not claim to clear WebView-internal storage/cookies beyond the existing AndroidX profile lifecycle contract.
...
## Current blockers
- GitHub Actions hosted jobs still fail during job startup before the first step, including minimal runner diagnostics and normal Unit/Runtime workflows. Fresh jobs show no useful step logs or assigned runner, so the blocker is external to Android/Gradle execution.
...
- Per-profile proxy routing remains architectural and is intentionally deferred.
